...
class SPGenDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sp = np.array(
            [self.start_token] + [SPGEN_AA_TO_ID[x] for x in sp] + [self.stop_token]
        )
        prot = np.array(
            [self.start_token] + [SPGEN_AA_TO_ID[x] for x in prot] + [self.stop_token]
        )
    ...
    def collate_fn(batch):
        ...
        prot_positions = torch.nn.utils.rnn.pad_sequence(
            prot_positions, batch_first=True, padding_value=0
        )
        sp_positions = torch.nn.utils.rnn.pad_sequence(
            sp_positions, batch_first=True, padding_value=0
        )
