...
def generate_sequence(self, src, org, stop_tokens=[21], max_len = 100, num_samples:int = 1, no_prot = False, no_org = False, top_p: float = 0):
    ...
    src_mask, _, src_padding_mask, tgt_padding_mask = create_mask(src, src, self.pad_idx)

    if no_prot:
        hidden_state_enc = torch.zeros_like(src_emb[[0],:,:]) #vanilla pytorch decoder stack needs memory.
    ...
    def _decode(tgt):
        _, tgt_mask, _, tgt_padding_mask = create_mask(src, tgt, self.pad_idx, tgt_additional_tokens= 0 if no_org else 1)
    ...
    y = torch.ones(num_samples, device=org.device, dtype=org.dtype) * 13 # start with M.
    ...
    for i in tqdm(range(max_len-1), leave=False):
        ...
        if top_p > 0.0:
            ...
            sampled_sorted_inds = sorted_probs.multinomial(num_samples=1, replacement=True)
    ...
    for idx in range(aa_logits.shape[1]):
        l = torch.nn.functional.cross_entropy(aa_logits[:,idx].reshape(-1, self.aa_vocab_size), sps_tgt[:,idx].reshape(-1), reduction='mean',ignore_index=0).item()
        ppl.append(np.exp(l))
...
def main():
    ...
    parser.add_argument('--max_aas', type=int, default=120, help='seq will be trimmed to this - SecretoGen was trained on 120')
    parser.add_argument('--batch_size', type=int, default=100, help='Number of samples to generate in parallel')
    parser.add_argument('--num_samples', type=int, default=1000, help='Number of samples to generate in total')
    ...
    prot = torch.LongTensor(np.array([AA_TO_ID[x] for x in args.seq[:args.max_aas]] + [AA_TO_ID['<eos>']])).unsqueeze(0).to(device)
    org = torch.LongTensor(taxonomy_labels.get_taxonomy_labels(args.org_id, levels_to_return=levels_to_use)).unsqueeze(0).to(device)
    ...
    with torch.no_grad():
        ...
        for _ in tqdm(range(n), total=n):
            preds, ppl = generate_sequence(model, prot, org, stop_tokens=stop_tokens, num_samples=args.batch_size, no_prot=False, no_org=False, top_p=args.top_p)
