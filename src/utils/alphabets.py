...
def make_aa_probs_from_codon_probs(
    codon_probs: torch.Tensor, start_codon: str = "ATG"
) -> torch.Tensor:
        ...
        for f in os.listdir(taxonomy_mapping_dir):
            ...
            if src != "species":
                raise NotImplementedError(
                    "Only accept species_to_X mappings at the moment."
                )
            ...
            self.values_to_class_ints[tgt] = {
                ID: INT + 1 for INT, ID in enumerate(tgt_vals_unique)
            }

        self.values_to_class_ints["species"] = {
            ID: INT + 1 for INT, ID in enumerate(d.keys())
        }

    def get_taxonomy_labels(
        self, species_id: int, levels_to_return: List[str]
    ) -> np.ndarray:
