...
def predict_spgen(model, loader):
    ...
    with torch.no_grad():
        ...
        for idx, batch in tqdm(enumerate(loader), total=len(loader)):
            ...
            proteins, prot_positions, sps, sp_positions = (
                proteins.to(device),
                prot_positions.to(device),
                sps.to(device),
                sp_positions.to(device),
            )

            aa_logits = model(
                (proteins, prot_positions), (sps, sp_positions)
            )  # sp_len -1, last pos trimmed

            ppls = get_perplexity_batch(
                model, proteins, prot_positions, sps, sp_positions
            )
...
def main():
    ...
    parser.add_argument(
        "--data", type=str, default="../data/fitness_data/grasso_prepared.csv"
    )
