...
def create_mask(src, tgt, pad_idx, tgt_additional_tokens=0):
    ...
    tgt_mask = generate_square_subsequent_mask(tgt_seq_len, src.device, src.dtype).type(
        torch.bool
    )
...
class PhylogenyEmbedding(nn.Module):
    def __init__(self, level_sizes: List[int], embedding_size: int, padding_idx=0):
        ...
        self.embedding_layers = nn.ModuleList(
            [TokenEmbedding(x, embedding_size, padding_idx) for x in level_sizes]
        )
...
class Seq2SeqTransformer(nn.Module):
    ...
    def forward(
        ...
        src_mask, tgt_mask, src_padding_mask, tgt_padding_mask = create_mask(
            src, trg, self.pad_idx, tgt_additional_tokens=1 if org is not None else 0
        )
        memory_key_padding_mask = src_padding_mask

        if len(self.transformer.encoder.layers) > 0:
            hidden_state_enc = self.transformer.encoder(
                src_emb, src_mask, src_padding_mask
            )
        else:
    ...
    def generate_sequence(self, src, org, max_len=70):
        ...
        for i in range(max_len - 1):
            ...
            y = torch.cat(
                [y, torch.ones(1, 1).type_as(src.data).fill_(next_token)], dim=0
            )
