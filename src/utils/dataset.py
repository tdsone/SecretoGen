...
class Seq2SeqDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.sps = (
            df["signal_peptide_dna"].astype(str).values
            if use_codons
            else df["signal_peptide_aa"].astype(str).values
        )
        self.proteins = (
            df["mature_protein_dna"].astype(str).values
            if use_codons
            else df["mature_protein_aa"].astype(str).values
        )
    ...
    def __getitem__(self, idx):
        ...
        sp = (
            string_to_codon_tokens(sp) if self.use_codons else [AA_TO_ID[x] for x in sp]
        )
        prot = (
            string_to_codon_tokens(prot)
            if self.use_codons
            else [AA_TO_ID[x] for x in prot]
        )
        ...
        orgs = self.taxonomy_labels.get_taxonomy_labels(
            org, levels_to_return=self.levels_to_use
        )
    ...
    def collate_fn(batch):
        ...
        proteins = torch.nn.utils.rnn.pad_sequence(
            proteins, batch_first=True, padding_value=0
        )
        sps = torch.nn.utils.rnn.pad_sequence(sps, batch_first=True, padding_value=0)
        prot_masks = torch.nn.utils.rnn.pad_sequence(
            prot_masks, batch_first=True, padding_value=1
        )
        sp_masks = torch.nn.utils.rnn.pad_sequence(
            sp_masks, batch_first=True, padding_value=1
        )
